...
// Strings come from window.TRANSLATIONS via useLang(). Proper nouns and
// abbreviations (program names, USA, J1, CIEE, DS-2019) stay identical.
...
// Non-translatable program metadata. Names, prices and visuals stay the same
// across languages; tag, duration and desc come from translations.
const PROGRAMS = [
    ...
    <section id="programs" style={{ paddingTop: embedded ? 96 : 'calc(var(--header-h) + 96px)' }}>
      <div className="container">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 20 }} className="programs-grid">
          {PROGRAMS.map((p) => {
            const tx = t.programs.items[p.id] || {};
            return (
              <div key={p.id} className="card" style={{ overflow: 'hidden', display: 'flex', flexDirection: 'column', height: '100%' }}>
                <div style={{ backgroundImage: `url(${p.bgImage})`, backgroundSize: 'cover', backgroundPosition: 'center', color: p.textColor, padding: '24px 22px', position: 'relative', height: 200, display: 'flex', flexDirection: 'column', flexShrink: 0, overflow: 'hidden' }}>
                  ...
                  <div style={{ fontSize: 11, letterSpacing: '0.12em', fontWeight: 700, textTransform: 'uppercase', opacity: 0.95, position: 'relative', zIndex: 1, color: 'white', textShadow: '0 1px 3px rgba(0,0,0,0.5), 0 0 8px rgba(0,0,0,0.3)' }}>{tx.tag}</div>
                ...
                <div style={{ padding: 22, display: 'flex', flexDirection: 'column', gap: 14, flex: 1, background: 'white' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--blue)' }}>{tx.duration}</div>
                    {p.cost && <div style={{ fontSize: 13, fontWeight: 800, color: 'var(--orange)' }}>{p.cost}</div>}
                  </div>
                  <p style={{ fontSize: 14, color: 'var(--ink-soft)', lineHeight: 1.55, flex: 1 }}>{tx.desc}</p>
...
// Team members: names + photo paths stay in code; role and bio come from translations indexed by position.
const TEAM_NAMES = [
  'Manuel Castillo',
  'Julieta Estrada',
  'Mariana Gutiérrez',
      ...
      <div className="container">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 20 }} className="team-grid">
          {TEAM_NAMES.map((name, i) => {
            const m = (t.team.members && t.team.members[i]) || { role: '', bio: '' };
            return (
              <div key={i} className="team-card" style={{ background: 'white', borderRadius: 'var(--radius)', overflow: 'hidden', border: '1px solid var(--line)' }}>
                ...
                <div style={{ padding: 22 }}>
                  ...
                  <h3 style={{ marginTop: 6, fontSize: 22 }}>{name}</h3>
...
// Testimonials: real names + locations stay; quote text translates.
      ...
      <div className="container">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20 }} className="testimonials-grid">
          {TESTIMONIAL_META.map((meta, i) => {
            const item = (t.testimonials.items && t.testimonials.items[i]) || { q: '' };
            return (
              <div key={i} style={{ background: 'white', border: '1px solid var(--line)', borderRadius: 'var(--radius)', padding: 28, display: 'flex', flexDirection: 'column' }}>
                ...
                <p style={{ fontSize: 16, lineHeight: 1.55, marginTop: -16 }}>{item.q}</p>
                <div style={{ marginTop: 'auto', paddingTop: 24, display: 'flex', gap: 14, alignItems: 'center' }}>
                  <Placeholder label="" tone={meta.tone} ratio="1" style={{ width: 52, height: 52, borderRadius: 999, padding: 0 }} />
                  <div>
                    <div style={{ fontWeight: 700 }}>{meta.name}</div>
                    <div style={{ fontSize: 13, color: 'var(--ink-soft)' }}>{meta.uni}</div>
                  </div>
                </div>
              </div>
            );
...
// Contact card icons stay in code matched by index.
const CONTACT_META = [{ icon: 'location' }, { icon: 'phone' }, { icon: 'mail' }];
            ...
            {t.contact.cards.map((x, i) => (
              <div key={i} style={{ display: 'flex', gap: 16, alignItems: 'flex-start', padding: '18px 0', borderBottom: i < 2 ? '1px solid rgba(255,255,255,0.15)' : 'none' }}>
                <div style={{ width: 44, height: 44, borderRadius: 12, background: 'var(--yellow)', color: 'var(--ink)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <Icon name={CONTACT_META[i].icon} size={20} />
                </div>
                <div>
...
// HasEmployer card metadata (icon, bg, phone) stays in code; title/desc/msg from translations.
const HAS_EMPLOYER_META = [
    ...
    <section style={{ paddingTop: 96, paddingBottom: 96 }}>
      <div className="container">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20 }} className="programs-grid">
          {HAS_EMPLOYER_META.map((meta, i) => {
            const card = (t.hasEmployer.cards && t.hasEmployer.cards[i]) || { title: '', desc: '', msg: '' };
            return (
              <a
                key={i}
                href={waUrl(meta.phone, card.msg)}
                target="_blank"
                rel="noopener noreferrer"
                className="card"
                style={{ background: meta.bg, padding: 28, display: 'flex', flexDirection: 'column', gap: 14, textDecoration: 'none', color: 'var(--ink)', minHeight: 240 }}
              >
                ...
                <div style={{ marginTop: 'auto' }}>
                  <h3 style={{ fontSize: 22, lineHeight: 1.15 }}>{card.title}</h3>
                  <p style={{ marginTop: 8, fontSize: 14, color: 'var(--ink)', opacity: 0.8 }}>{card.desc}</p>
                </div>
...
// Terms uses HTML in some items (<b>tags</b>) — render via dangerouslySetInnerHTML.
const Terms = () => {
  const { t } = useLang();
      ...
      <div className="container" style={{ maxWidth: 860 }}>
        ...
        <div className="terms-content" style={{ marginTop: 48, display: 'flex', flexDirection: 'column', gap: 32, fontSize: 15, lineHeight: 1.7, color: 'var(--ink-soft)' }}>
          ...
          <section>
            ...
            <ol style={{ marginTop: 12, paddingLeft: 20, display: 'flex', flexDirection: 'column', gap: 10 }}>
              {s.responsibilities.items.map((it, i) => (<li key={i}>{it}</li>))}
            </ol>
          </section>
          ...
          <section>
            ...
            <ul style={{ marginTop: 12, paddingLeft: 20, display: 'flex', flexDirection: 'column', gap: 8 }}>
              {s.cost.items.map((it, i) => (<li key={i}>{it}</li>))}
            </ul>
...
// Employers metadata (visuals) stays in code. Categories, name and subtitle from translations.
const EMPLOYERS = {
  swt: { name: 'Summer Work & Travel', color: 'var(--yellow)', textColor: 'var(--ink)', icon: 'sun', tone: 'yellow' },
  camp: { name: 'Camp Exchange', color: 'var(--green)', textColor: 'white', icon: 'tent', tone: 'green' },
  intern: { name: 'Internship & Trainee', color: 'var(--blue)', textColor: 'white', icon: 'briefcase', tone: 'sky' },
  support: { name: 'Support Staff', color: 'var(--orange-soft)', textColor: 'white', icon: 'chef', tone: 'orange', hasSubtitle: true },
};
    ...
    <section style={{ paddingTop: embedded ? 96 : 'calc(var(--header-h) + 96px)' }}>
      <div className="container">
        ...
        {Object.entries(EMPLOYERS).map(([key, prog]) => {
          const categories = (t.employers.categories && t.employers.categories[key]) || [];
          const subtitle = prog.hasSubtitle ? (t.employers.subtitles && t.employers.subtitles[key]) : null;
          return (
...
// Standalone wrappers for tabs — labels come from translations at render time
const ProgramsPage = (props) => <main data-screen-label="Programs"><Programs {...props} /></main>;
const EmployersPage = (props) => <main data-screen-label="Employers"><Employers {...props} /></main>;
const VisaPage = (props) => <main data-screen-label="Visa"><Visa {...props} /></main>;
const ProcessPage = (props) => <main data-screen-label="Process"><Process {...props} /></main>;
const TeamPage = (props) => <main data-screen-label="Team"><Team {...props} /></main>;
const TestimonialsPage = (props) => <main data-screen-label="Testimonials"><Testimonials {...props} /></main>;
const FAQPage = (props) => <main data-screen-label="FAQ"><FAQ {...props} /></main>;
const ContactPage = (props) => <main data-screen-label="Contact"><Contact {...props} /></main>;
